...
<body>
    ...
    <section class="text-gray-600 body-font bg-gray-100">
        <div class="container px-5 py-24 mx-auto flex flex-col items-center">
            <h1 class="text-3xl font-semibold text-gray-900 mb-6">Create a New To-Do Item</h1>

            <form action="{{ route('todolist.store') }}" method="POST" class="w-full max-w-lg bg-white p-8 rounded-lg shadow-lg">
                @csrf

                <div class="mb-6">
                    <label for="titulli" class="block text-gray-700 text-sm font-medium mb-2">Title:</label>
                    <input type="text" name="titulli" id="titulli" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" required>
                </div>

                <div class="mb-6">
                    <label for="pershkrimi" class="block text-gray-700 text-sm font-medium mb-2">Description:</label>
                    <textarea name="pershkrimi" id="pershkrimi" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" required></textarea>
                </div>

                <button type="submit" class="w-full bg-green-500 text-white py-2 px-4 rounded-full shadow-lg hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 transition-all duration-300 ease-in-out">Save</button>
            </form>
